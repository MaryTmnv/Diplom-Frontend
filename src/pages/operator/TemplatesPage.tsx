...
import { Sparkles, FileQuestion, FileText, LayoutGrid, LayoutList, Lightbulb, List, Plus, Search, Tag } from 'lucide-react';

function TemplatesPage() {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedCategory, setSelectedCategory] = useState<TicketCategory | undefined>();
  const [showTemplateSelector, setShowTemplateSelector] = useState(false);
  const [selectorCategory, setSelectorCategory] = useState<TicketCategory | undefined>();
  ...
  const handleUseTemplate = (template: Template) => {
    // Копируем содержимое шаблона в буфер обмена
    navigator.clipboard.writeText(template.content);
    alert(`Шаблон "${template.title}" скопирован в буфер обмена! Вставьте его в чат.`);
  };

  const handleOpenTemplateSelector = (category?: TicketCategory) => {
    setSelectorCategory(category);
    setShowTemplateSelector(true);
  };

  const handleTemplateSelect = (template: Template) => {
    // Копируем содержимое выбранного шаблона
    navigator.clipboard.writeText(template.content);
    alert(`Шаблон "${template.title}" скопирован в буфер обмена!`);
        ...
        <Card className="border-0 shadow-lg rounded-2xl overflow-hidden bg-gradient-to-br from-amber-50/50 to-yellow-50/30 relative">
          ...
          <CardContent className="relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {popularTemplates.map((template: Template, index: number) => (
                <div 
                  key={template.id}
