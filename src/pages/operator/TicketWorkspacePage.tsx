...
import { TicketStatus, type Ticket, type Attachment, type InternalNote } from '@/features/tickets/types/tickets.types';
...
import { 
  ArrowLeft, Clock, CheckCircle, XCircle, Tag, Calendar, MessageSquare, 
  FileText, BookOpen, ChevronDown, ChevronUp, Download, ExternalLink, 
  FileIcon, Lightbulb, MessageCircle, MoreVertical, Paperclip, Phone, 
  PlayCircle, Plus, Settings2, Star, StickyNote, User, Zap, History 
} from 'lucide-react';
...
import type { Article } from '@/features/knowledge-base/types/article.types';
...
export const TicketWorkspacePage = () => {
  ...
  const [___, setShowTemplateSelector] = useState(false);
  const [__, setMessageInput] = useState('');
              ...
              <div className="flex items-center gap-3 p-3 bg-[#caf0f8]/20 rounded-xl">
                ...
                <div>
                  ...
                  <p className="font-semibold text-[#03045e]">
                    {formatDate(new Date(ticket.createdAt), 'dd.MM.yyyy HH:mm')}
                  </p>
                </div>
              </div>
                    ...
                    >
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-xs text-[#023e8a]/50">
                          {formatDate(new Date(note.createdAt), 'dd.MM HH:mm')}
                        </span>
                      </div>
                ...
                {ticket.suggestions?.similarTickets && ticket.suggestions.similarTickets.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-2">
                      {ticket.suggestions.similarTickets.slice(0, 3).map((similar: Ticket) => (
                        <button
                          key={similar.id}
  ...
  <div>
    ...
    <div className="space-y-2">
      {ticket.suggestions.articles.slice(0, 3).map((article: Article) => (
              <a
                key={article.id}
                ...
                {ticket.suggestions.templates && 
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {ticket.suggestions.templates.slice(0, 3).map((template: Template) => (
                        <Button
                          key={template.id}
                          variant="outline"
                          size="sm"
